...
**I decoded the string and found it was a set of credentials (admin:admin). I thought these might not work, so I searched for default credentials for WebDAV and tested a few combinations. After some trial and error, I discovered that the credentials worked, giving me access to the WebDAV server.**

**Not wanting to waste too much time searching, I asked ChatGPT for guidance. That’s when I learned that WebDAV server was vulnerable to `file uploads` an attacker can manipulate files if they have the right credentials, So I decided crafting php webshell using curl athentication. After that I successfully uploaded a payload and gained initial access.**
